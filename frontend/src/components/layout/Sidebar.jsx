        ...
        {navItems.map(item => {
          ...
          return (
            <button key={item.id} onClick={() => setView(item.id)} style={{
              ...
              <SVG name={item.icon} size={19} color={active ? 'var(--accent)' : 'var(--text-4)'} />
              <span style={{ fontSize: 15, fontWeight: active ? 500 : 400 }}>{item.label}</span>
            </button>
          );
        })}
      </nav>
      ...
      <div style={{ padding: '14px 14px 16px', borderTop: '1px solid var(--border-1)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <p style={{ fontSize: 14, fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{user.nombre}</p>
            <p style={{ fontSize: 12, color: role.color }}>{role.label}</p>
          </div>
        </div>
      </div>
    </aside>
